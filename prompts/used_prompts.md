...
## #22 프로젝트 구조 README.md 파일 생성 요청
**프롬프트**
```
혹시 현재까지 구현 된 프로젝트를 기준으로 README.md 를 잘 요약해서 정리를 해 줄 수 있을까 ? PDF 명세서 파일대로 정리 요청한 중요한 순으로 정리를 하는게 나을 것 같아서 실행방법, API 명세 요약 부터 우선 정리를 해줘 ! 프로젝트를
처음 보는 사람도 바로 이해할 수 있도록 잘 정리를 해주었으면 좋겠어 ! 그리고 아키텍처랑 ERD 같은 것 프로젝트의 이해를 도와 줄 수 있는  플로우 차트나 그림으로 가시성 있게 잘 정리를 해주었으면 좋겠어 !
```

## #23 README.md API 명세 curl 생성 요청
**프롬프트**
```
혹시 README.md API 명세서에 각 API 를 간단히 바로 확인해 볼 수 있도록 curl 로 요청하는 방법도 추가해 주었으면 좋겠어 !         
```
