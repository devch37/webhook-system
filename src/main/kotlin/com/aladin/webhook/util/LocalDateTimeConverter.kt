...
class LocalDateTimeConverter : AttributeConverter<LocalDateTime, String> {
    companion object {
        ...
        private val WRITE_FORMATTER = DateTimeFormatterBuilder()
            .appendPattern("yyyy-MM-dd HH:mm:ss")
            .appendFraction(ChronoField.MILLI_OF_SECOND, 3, 3, true)
            .toFormatter()

        // 읽기: 밀리초 없는 기존 레코드("yyyy-MM-dd HH:mm:ss")도 파싱 가능
        private val READ_FORMATTER = DateTimeFormatterBuilder()
            .appendPattern("yyyy-MM-dd HH:mm:ss")
            .optionalStart()
    ...
    override fun convertToDatabaseColumn(attribute: LocalDateTime?): String? =
        attribute?.format(WRITE_FORMATTER)

    override fun convertToEntityAttribute(dbData: String?): LocalDateTime? =
        dbData?.let { LocalDateTime.parse(it, READ_FORMATTER) }
}
